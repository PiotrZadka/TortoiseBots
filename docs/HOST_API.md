...
**Goblin / High Elf spawn — closed / fixed:** `TravelNodeMap::generateStartNodes()` (`ai/playerbot/TravelNode.cpp`) and `PlayerbotAIConfig` legality derive start positions and race/class validity from core `PlayerInfo` (`sObjectMgr.GetPlayerInfo` over `playercreateinfo`); `RepopAction` in `ReleaseSpiritAction.h` uses that `PlayerInfo` position with `homebind` fallback only when no row exists. No `RACE_GOBLIN`→Durotar / `RACE_HIGH_ELF`→Elwynn donor hack is present and none should be re-added. Shipped `maps/0013245.map`+`mmaps/0013245.mmtile` (Goblin) and `maps/0002536.map`+`mmaps/0002536.mmtile` (High Elf) complete the data path — no module code or data change required. Movement/LoS acceptance and the separate High Elf `vmaps/000_25_36` gap remain distinct (tracked in `docs/archive/PLAYERBOTS_AUDIT.md` / `PROVENANCE.md`, audit F-06) and are not claimed as tested here.

**High Elf VMap — external data blocker (docs only, no code/data change):** Referenced runtime snapshot `tortoise-docker-penqle/data` (audited 2026-08-25): `maps/0002536.map` + `mmaps/0002536.mmtile` are present in the referenced runtime snapshot, `vmaps/000_25_36.vmtile` is absent (`vmaps/001_32_45.vmtile` present as control; same absence in `shyalya-tortoise-wow` at `2026-08-25T04:01:47`). Thalassian Highlands WMO is Turtle custom not in 1.18.1 build 7272 MPQ — `vmap_extractor` → `vmap_assembler` skips the `WDT` with no `WMO`; this is external Turtle extraction/data ownership, not module code. `Map::isInLineOfSight` → `VMapManager2::isInLineOfSight` keeps existing permissive VMap missing-tile fallback (returns `true` when no tile loaded); do not synthesize tiles or re-add race hacks. Do not claim movement/LoS runtime acceptance. No code/data change is claimed.
...
**Custom dungeon portal / unverified ScriptNames — core/data blocker (docs only, no code/data change):** `npc_teslinah` registration (`src/scripts/miscellaneous/random_scripts_1.cpp` `QuestAccept_npc_teslinah` — not `src/scripts/spells/spell_druid.cpp`) and literal `script_name='0'` cleanup (`20260825090000_world.sql` — literal value only) are separate, sanctioned, and distinct from unresolved custom `ScriptName` rows which remain open for triage against the current `archive/PLAYERBOTS_AUDIT.md` F-27 audit list; do not conflate the two. Unresolved `custom_dungeon_portal` and other Turtle ScriptNames are core/data-owned and unverified — require core `ScriptMgr` `OnGossipHello`/`OnQuestAccept` registration (or DB fix) before any module seam. No module fake `custom_dungeon_portal` handler, no `script_name='0'` fake success, no Karazhan `DungeonStrategy` re-add (excluded via `TortoiseBots.cmake` filter — do not re-add). Module follow-up (optional `DungeonStrategy`/`TravelMgr` waypoints for leading custom dungeons) only after real core scripts ship. Do not guess ScriptNames or add code/data. No code, core, or data change is claimed.
