...
// pi-lens-ignore: clang:unknown_typename
    ...
    // Pullback POC: tank runs toward target to shoot/body-aggro then returns to master's anchor and holds.
    bool RequestPullback(Player* requester, Player* tank, Unit* target, bool isRanged, float desiredDist);
    void CancelPullback(ObjectGuid tankGuid);
    bool IsPullbackActive(ObjectGuid tankGuid) const;

    // Summon POC: portal 3s at 5y then teleport bot and follow.
